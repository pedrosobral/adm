...
\noindent {\bf Notação:} Representamos um intervalo aberto pelas suas extremidades entre parenteses. Por exemplo, o intervalo aberto que compreende todos os números entre 2 e 5 é representado por $(2,5)$.
...
\begin{mybox}
\begin{quotation} \noindent {A função $f$ relaciona cada elemento $a$ do conjunto $A$ a um único elemento $b$ do conjunto $ B$. Denotaremos  $f(a)=b$.}\end{quotation}
\end{mybox}

\begin{ex} Abaixo um exemplo de uma relação que é função, pois satisfaz a condição acima, e outra que não é função.
...
%\fbox{
\begin{minipage}{7cm}
\noindent{\bf Hora do cafezinho:}

\wesa
A fun$\mbox{\it ç}$a$\tilde{}$o $f$ de $A$ em $B$ pode ser imaginada como uma m\ag{a}quina que usa como mat\ag{e}ria prima os elementos de $A$ e produz elementos de $B$.
Por exemplo, se $f$ \ag{e} uma determinada m\ag{a}quina de caf\ag{e} expresso de c\ag{a}psula, cujo fabricante disponibiliza os segintes tipos de c\ag{a}psulas:
...
A cada tipo de c\ag{a}psula $f$ associa um tipo de caf\ag{e}.
O conjunto $A$ \ag{e} o dom\ag{i}nio de $f$.
O contra dom\ag{i}nio ser\ag{a}, digamos, o conjunto $B$ de todos os sabores de caf\ag{e} que existem.
Por\ag{e}m a imagem, $\Im{f}$, \ag{e} a cole$\mbox{\it ç}$a$\tilde{}$o que cont\ag{e}m apenas o sabores de caf\ag{e} que sa$\tilde{}$o de fato produzidos pela m\ag{a}quina, isto \ag{e},
...
\noindent {\bf  Plano Cartesiano:} O plano cartesiano pode ser construído a partir de qualquer plano dado da seguinte forma:
...
\noindent {\bf Grafico de uma função:} Seja $f$ uma função com domínio e contra domínio no conjunto dos números reais. Então o gráfico de $f$ é o seguinte subconjunto de $\R^2$,
...
\noindent {\bf Equação do Primeiro Grau:} Uma equação do primeiro grau é uma expressão matemática do tipo:
$$ax+b=c$$
...
\noindent{\bf Gráfico da funçãoo de primeiro grau:} O gráfico da função de primeiro grau é
...
\noindent{\it Break Even Point:} é o ponto de equilíbrio entre custo total e receita, isto é $C(x)=R(x)$. No break even point o lucro será zero.

\vspace{0.5cm}

\noindent{\bf Graficamente:} Tanto o custo total quanto a receita são dados por retas. O break even point é o ponto onde estas retas se cruzam.


\begin{center}
\includegraphics{breakeven}
\end{center}
...
\noindent{\bf Classificação:} Um sistema linear pertence obrigatóriamente a uma das três categorias abaixo:
...
\noindent{\bf Equação do segundo grau:} É uma equação da forma $ax^2+bx+c=0$. Uma solução é um valor real $r$ tal que quando substituímos $x=r$ na equação, a igualdade é verdadeira. Uma equação
...
\noindent {\bf Gráfico da função do segundo grau:}O gráfico de uma função do segundo grau é uma parábola. Considere a função
...
\begin{center}
\begin{tabular}{cc}
\includegraphics{SadHappy.jpg} & \includegraphics{Happy.jpg} \\
$a<0$ & $a>0$
\end{tabular}
\end{center}
\end{multicols}
...
\noindent{\bf Função polinomial ou polinômio:}  É uma função da forma:
...
\noindent{\bf OBS.:}Quando não é feita uma restrição sobre o domínio de $f$ podemos admitir que o domínio é toda a reta real $\R$, uma vez que uma fórmula do tipo
...
\noindent{\bf Função Racional:} É uma função cuja fórmula é dada pelo quociente de dois polinômios, ou seja:
\begin{equation}
...
\noindent{\bf Funções dadas por Radicais:}  A função raíz n-ésima é a função inversa da função n-ésima potência desde que
...
\begin{center}
\begin{minipage}{16cm}\fbox{\noindent{ \bf OBS.:} Raízes de ordem par apenas estão definidas para números não negativos.
}
\end{minipage}
\end{center}
...
Duas grandezas $x$ e $y$ são {\it proporcionais} se existe uma constante não nula $k\in \mbox{R}\setminus \{0\}$ tal que $y=kx$. Note que é um caso particular de função do primeiro grau, onde o coeficiente linear é zero.

Duas grandezas $x$ e $y$ são {\it inversamente proporcionais} se existe uma constante não nula $k$ tal que $y=k\frac{1}{x}$. Note que neste caso $y$ é proporcional ao inverso de $x$.

Apresentamos nos exemplos abaixo alguns problemas típicos envolvendo proporcionalidade.

\begin{ex} Em uma fabrica de camisetas, 10 trabalhadores produzem 160 camisetas por dia. Quantos trabalhadores serão necessários para produzir 240 camisetas por dia?
...
\noindent{1)} Determine se a relação expressa em cada um dos seguintes itens é uma função. Indique quando a função for injetiva, sobrejetiva ou bijetiva.
...
\begin{center}
\includegraphics[scale=0.8]{naofuncao}
\end{center}
...
\noindent{2)} Resolva a equação, se houver solução:
...
\noindent {\bf 3)} Resolva o sistema linear, clssificando-o como possuindo solução única, infinitas soluções,
